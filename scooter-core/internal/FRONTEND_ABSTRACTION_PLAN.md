...
### 1. `AppRuntime` trait (Phase 0.3) — possibly over-engineering

The goal of Phase 0.3 is that frontends don't need to participate in Tokio internals. But that goal is already achieved by `drain_effects()`: as long as the public API doesn't expose `tokio::select!` or `UnboundedReceiver`, frontends don't need to know Tokio is involved at all. `scooter-core` can continue using Tokio internally without it being part of the contract.

A formal `AppRuntime` trait adds real costs:
- thread safety bounds (`Send + Sync + 'static`) that constrain the implementation
- dynamic dispatch overhead
- complexity for anyone reading or maintaining the scheduler abstraction
- no clear current payoff if there's no concrete alternative runtime to swap in

Proposal: drop Phase 0.3, or defer it until there's a concrete reason to swap runtimes (e.g. a WASM target or a synchronous test executor). Note in the design that the internal implementation uses Tokio but that this is an implementation detail not part of the public contract.

### 2. Owned DTO cloning cost (Phase 2)

Returning owned `AppView` structs (rather than lifetime-bound references) is the right call for FFI and plugin bridges. However, cloning `Vec<SearchResultView>` on every call to `view()` could be expensive in the TUI case, which may call it at 60fps with thousands of results.

Two options worth considering:

- **Clone on demand, not on every frame.** The TUI should only call `view()` when it actually needs to re-render (i.e. when `drain_effects()` returned something, or when a key event was handled). This avoids gratuitous clones without changing the API.
- **Keep results as indices/IDs in the view, lazy-fetch content.** `SearchProgressView` could hold a result count and selection state, with individual `SearchResultView`s fetched on demand via `App::result(id)`. This avoids cloning the whole list but adds API surface.

The simplest approach is the first: keep the owned DTO design and document that `view()` should only be called when a rerender is actually needed. Worth deciding before implementation begins.
