...
use crate::replace::{self, ReplaceResult};
...
impl FileSearcher {
        ...
        F: FnMut() -> FileVisitor + Send,
    {
        ...
        walker.run(|| {
            ...
            Box::new(move |result| {
                ...
                {
                    // TODO: allow users to either do this:
                    if let Err(e) =
                        Self::search_and_replace_in_file(entry.path(), &self.search, &self.replace)
                    {
                        log::error!(
                            "Failed to search and replace in {path}: {e}",
    ...
    fn file_has_match(path: &Path, file: &File, search: &SearchType) -> bool {
        let reader = BufReader::with_capacity(16384, file);
        ...
        for (mut line_number, line_result) in reader.lines_with_endings().enumerate() {
            ...
            let (line_bytes, _line_ending) = match line_result {
                Ok(l) => l,
                Err(err) => {
                    read_errors += 1;
                    log::warn!(
    ...
    fn search_and_replace_in_file(
        ...
        let mut file = match File::open(path) {
            Ok(f) => f,
            Err(err) => {
                bail!("Error opening file {}: {err}", path.display());
            }
        };
        ...
        if Self::file_has_match(path, &file, search) {
            replace::replace_in_open_file(path, &mut file, search, replace)?;
        }
...
fn line_has_match(line: &str, search: &SearchType) -> bool {
    if line.is_empty() || search.is_empty() {
        return false;
    }

    match search {
        SearchType::Fixed(ref fixed_str) => line.contains(fixed_str),
        SearchType::Pattern(ref pattern) => pattern.is_match(line),
        SearchType::PatternAdvanced(ref pattern) => pattern.is_match(line).unwrap_or(false),
    }
}
...
pub fn replacement_if_match(line: &str, search: &SearchType, replace: &str) -> Option<String> {
    ...
    match search {
        SearchType::Fixed(ref fixed_str) => Some(line.replace(fixed_str, replace)),
        SearchType::Pattern(ref pattern) => Some(pattern.replace_all(line, replace).to_string()),
        SearchType::PatternAdvanced(ref pattern) => {
...
mod tests {
    ...
    mod unicode_handling {
        ...
        fn test_complex_unicode_replacement() {
            ...
            let result = replacement_if_match(text, &search, "World");
        ...
        fn test_unicode_word_boundaries() {
            ...
            assert!(replacement_if_match("Text 世界 more", &converted, "XX").is_some());
            assert!(replacement_if_match("Text世界more", &converted, "XX").is_none());
        }
        ...
        fn test_unicode_normalization() {
            ...
            assert_eq!(
                replacement_if_match(text, &search, "e"),
                Some("cafe".to_string())
            );
        }
        ...
        fn test_unicode_regex_classes() {
            ...
            assert_eq!(
                replacement_if_match(text, &search, "X"),
        ...
        fn test_unicode_capture_groups() {
            ...
            assert_eq!(
                replacement_if_match(text, &search, "ID $2 belongs to $1"),
